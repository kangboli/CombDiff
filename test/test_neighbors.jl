...
@testset "neighbors: primitive calls with symmetries" begin
    ...
    @test all(f -> fc(f) in nodes(PCT.neighbors(fc(f1); settings=symmetry_settings)), [f2, f3, f4])
    ...
    @test fc(f2) in nodes(PCT.neighbors(fc(f1); settings=symmetry_settings))
    @test fc(f3) in nodes(PCT.neighbors(fc(f1); settings=symmetry_settings))
end
...
@testset "neighbors: addition" begin
    ...
    @test fc(f1) in nodes(PCT.neighbors(fc(f2); settings=symmetry_settings))
end
...
@testset "neighbors: monomial" begin
    ...
    @test fc(f2) in nodes(PCT.neighbors(fc(f1)))
    @test fc(f3) in nodes(PCT.neighbors(fc(f1)))
...
@testset "neighbors: constraction" begin
    ...
    @test fc(f2) in nodes(PCT.neighbors(fc(f1); settings=custom_settings(:clench_sum=>true)))
