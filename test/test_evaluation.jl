...
@testset "substitution" begin
    ...
    f1, _ = @pct f ctx i * sum(i, A(i, j))
    f2, _ = @pct f ctx k * sum(i, A(i, j))
    @test get_body(f2) == subst(get_body(f1), var(:i, I()), var(:k, I()))

    f1, _ = @pct f ctx (a::I) -> A(a, j)
    @test get_body(f1) == subst(get_body(f1), var(:a, I()), var(:i, I()))

    # Resolving conflict

    f1, _ = @pct f ctx k * sum(i, A(i, j))
    f2, _ = @pct f ctx k * sum(a, A(a, i))
    @test get_body(f2) == subst(get_body(f1), var(:j, I()), var(:i, I()))

    f1, _ = @pct f ctx i * sum(i, sum(j, A(i, j) * k))
    f2, _ = @pct f ctx i * sum(a, sum(b, A(a, b) * i * j))
