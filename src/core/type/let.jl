...
is_split(n::WithMemory) = is_split(get_body(n))
...
split_let(n::WithMemory) = with_memory(get_memory(n), split_let(n))

function split_let(l::Let)
    foldr(((b, a), body) -> pct_let(b, a, body), zip(get_bound(l), args(l)); init=split_let(get_body(l)))
end

is_combined(m::WithMemory) = is_combined(get_body(m))

is_combined(l::Let) = !isa(get_body(l), Let) 

combine_let(n::WithMemory) = with_memory(get_memory(n), combine_let(get_body(n)))
...
function let_to_call(l::Let)
    ...
    primitive_call(pct_map(get_body.(get_bound(l))..., let_to_call(get_body(l))), args(l)...)
end

call_to_let(n::APN) = n

call_to_let(n::WithMemory) = with_memory(get_memory(n), call_to_let(get_body(n)))
